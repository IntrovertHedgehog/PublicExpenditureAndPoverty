...
```
Some significant correlations can be found between `gdr.eql` with `edu.pri`, `pop.gwth.total`, and `fdi` with `mil`, etc.  
We expect these variables to be eliminated in the VIF test.  
Log transform some variables.  
```{r}
countries1 <- countries1 %>% 
  mutate(lfdi = log(fdi), lgdp.dflt = log(gdp.dflt), lgdp.pc = log(gdp.pc)) %>% 
  mutate(lfdi = ifelse(is.nan(lfdi) | lfdi == -Inf, NA, lfdi)) # for lack of better measure, Inf and NaN value of lfdi will be ignored
```
