...
needs("dplyr", "MASS", "knitr", "readr", "tidyr", "ggplot2", "e1071", "moments", "corrplot", "Hmisc", "PerformanceAnalytics", "mice")
...
set.seed(1984)
# split data
idx <- sample(1:nrow(countries1), replace = F, 0.7 * nrow(countries1))
countries.train.4 <- countries1[idx,]
countries.test.4 <- countries[-idx,]
...
# run the algorithm
# countries1.imputed <- mice(countries1, m = 5, maxit = 30, method = meth)
...
We generated 5 sets of imputated (training) data. We can build our model on each of these data sets and combine the estimates using [pooling rule](https://amices.org/mice/reference/pool.html#details). We remove `reg` and `country.name` from the model because they're perfectly predicted by `country.code`

We can start with using one of the imputated sets.
```{r}
fit1 <- lm(pov ~ .-country.name, countries1.imputed.1)
summary(fit1)
```

We might be more interested with large regions of countries rather than individual countries.
```{r}
fit1reg <- lm(pov ~ .-country.name-country.code, countries1.imputed.1)
summary(fit1reg)
```

With all the generated data sets.

```{r}
fit2 <- with(data = countries1.imputed, exp = lm(pov ~ reg+year+income+edu.total+hlth+mil+fdi+lbr.part+unemp+pop.gwth.total+pop.gwth.rural+pop.gwth.urban+gdp.dflt+gcf+trade+gdp.pc+lfdi+lgdp.dflt+lgdp.pc))
fit2.combined <- pool(fit2)
summary(fit2.combined)
pool.r.squared(fit2.combined)
```

We can conduct a step-wise AIC variable selection. It is similar to the procedure we use in class, but based on a metric call AIC (Akaike Information Criterion), which is an estimator of prediction error and relative quality of statistical models.  

continue with fitting and displaying model [link](https://thomaselove.github.io/432-notes/multiple-imputation-and-linear-regression.html#fit-linear-regression-using-ols-and-fit.mult.impute)  
