...
## 3.1. Use Complete Cases (To be done)
###  3.1.1. Model Fitting
###  3.1.2. Assessment
###  3.1.3. Interpretation

## 3.2. Selectively remove variables with high missing rate (To be done)
**1. About selectively removing variables with high missing rate**
...
subset1 <- subset0 %>% mutate(lfdi = log(subset0$fdi)) %>% mutate(lgdp.pc = log(subset0$gdp.pc)) %>% mutate(lgdp.dflt = log(subset0$gdp.dflt))

subset1 <- subset(subset1, select = c("edu.total", "lfdi","gcf","lgdp.dflt","hlth","income","lbr.part","mil","pop.gwth.rural","pop.gwth.urban","pov","trade", "unemp", "year","lgdp.pc"))
...
First, we split the data into training and testing data. The training dataset contains 80% of the initial dataset, and the testing data contains 20% of the whole dataset. 
```{r}
set.seed(123)
...
```
Our first model includes all the variables with missing rate lower than 35%. This model is assigned to `lm_1`.
```{r}
...
```{r}
lm_2 <- lm(data= train_1, formula = pov ~ . -lgdp.dflt)
summary(lm_2)
```
Removing `lfdi`,
```{r}
lm_3 <- lm(data= train_1, formula = pov ~ .-lfdi - lgdp.dflt)
summary(lm_3)
```
Further removing `hlth`,
```{r}
...
Later on, we will compare the results of all the models we generated.
```{r}
data0 <- subset1
data <- na.omit(data0)
...
test <- data[-sample, ]
```
Further dividing into `train.x`, `train.y`, `test.x`, and `test.y`.
```{r}
...
test.y <- test$pov
```
Using the lambda value that yields the lowest MSE value. 
```{r}
...
## 3.3. Update the data set as we select variables (To be done)
###  3.3.1. Model Fitting
###  3.3.2. Assessment
...
for (mod in models) {
  ...
  fitted <- mod$train$data$pov + mod$train$residuals
...
lapply(models, \(m) {
  ...
  gqtest(formula = resid ~ 1, order.by = m$train$data$pov)
})
```
...
As elastic net model produced in 
